...
( function( window, document, controllers, Form, D ) {
  ...
  const Select_FieldType = new Form.FieldType( 'Select_Field', {
	...
	const Calendar_FieldType = new Form.FieldType( 'Calendar_Field', {
		inputSelector: 'input',
		parseDispValue: function( dispValue ) {	return D.formatYmd( D.parseLong( dispValue ) ); },
		printDispValue: function(v) { return D.formatLong( D.parseYmd(v) ); },
		getValue: function() {
...
}( window, document, F1.controllers, F1.Form, F1.Date ) );